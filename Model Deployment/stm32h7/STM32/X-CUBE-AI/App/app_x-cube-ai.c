...
/* ????? */
__attribute__((section(".RAM_D1"), aligned(32))) ai_u8 input_data_u8[AI_NETWORK_IN_1_SIZE_BYTES];
__attribute__((section(".RAM_D1"), aligned(32))) ai_float output_data[AI_NETWORK_OUT_1_SIZE];


/* ?? main.c ??? */
extern __IO uint16_t detect_img_data[img_size * img_size];
extern char display_str[32];
extern char conf_str[32];

const char * class_names[] = {"100", "50", "50-100"};
...
static void Precompute_8IntU(float scale, int32_t zp,
                             float prepro_scale, int32_t prepro_zp)
{
    for (int i = 0; i < 256; i++) {
        // ??? [0,1]
        float x = ((float)(i - prepro_zp)) / prepro_scale;
        // ????
        int32_t q = (int32_t)llroundf((x / scale) + (float)zp);
        if (q < 0) q = 0;
        if (q > 255) q = 255;
        pixel_conv_lut[i] = (uint8_t)q;
    }
}
...
int ai_run(uint8_t *data_in, float *data_out)
{
    ...
    // ????????
    ai_input[0].data = AI_HANDLE_PTR(input_data_u8);
...
int acquire_and_process_data(void)
{
    ...
    for (int i = 0; i < img_size * img_size; i++) {
        ...
        // ? LUT ???? & ????
        *dst++ = pixel_conv_lut[r8];
        *dst++ = pixel_conv_lut[g8];
        *dst++ = pixel_conv_lut[b8];
    }
...
int post_process(void)
{
    ...
    snprintf(display_str, sizeof(display_str), "Class: %s", class_names[idx]);
    snprintf(conf_str, sizeof(conf_str), "Conf: %.2f%%", max_conf * 100.0f);
    ...
    int MX_X_CUBE_AI_Process(void)
    {
        ...
        if (network)
        {
           ...
           if (res == 0)
             res = ai_run(NULL, NULL);   // ????,??? input_data/output_data
